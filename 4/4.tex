...
\hypersetup{
  ...
  pdfauthor={Gemini},
  pdftitle={Лекция 4: Работа с памятью и процессами}
}
...
\newcommand{\LectureNo}{4}
\newcommand{\LectureTitle}{Работа с памятью и процессами}
\newcommand{\LectureDate}{05.10.2025}
...
\tikzset{
  box/.style={draw=Accent, rounded corners, fill=AccentLight, minimum width=2.2cm, minimum height=0.9cm, align=center, drop shadow={opacity=0.1, shadow xshift=1pt, shadow yshift=-1pt}},
...
\newacronym[sort=pid]{pid}{PID}{идентификатор процесса (Process ID)}
\newacronym[sort=os]{os}{ОС}{операционная система}

\newglossaryentry{vmem}{name={виртуальная память}, description={Абстракция памяти, которая предоставляет процессу иллюзию непрерывного и изолированного адресного пространства.}}
\newglossaryentry{pmem}{name={физическая память}, description={Реальная оперативная память (RAM), установленная в компьютере.}}
...
\newglossaryentry{pipe}{name={канал (pipe)}, sort={kanal}, description={Механизм межпроцессного взаимодействия, позволяющий перенаправлять стандартный вывод одного процесса на стандартный ввод другого.}}
...
\gls{pagefault} — это прерывание, которое генерируется аппаратно (процессором) при попытке доступа к странице \gls{vmem}, не имеющей корректного отображения в \gls{pmem}. При возникновении \gls{pagefault} исполнение текущего кода программы приостанавливается, и управление передаётся обработчику в \gls{os}.
\end{definitionbox}
