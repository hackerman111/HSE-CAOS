...
% PDF/Cyrillic search/copy compatibility
\usepackage{cmap}
...
\hypersetup{
  ...
  pdftitle={Лекция 8: Эффективный ассемблер, LTO и динамическая компоновка}
}
...
\pagestyle{fancy}
\fancyhf{}
% Metadata macros (override in document/LLM)
\newcommand{\CourseName}{Архитектура компьютера и ОС}
\newcommand{\LectureNo}{8}
\newcommand{\LectureTitle}{Эффективный ассемблер, LTO и динамическая компоновка}
\newcommand{\LectureDate}{08.11.2025}
...
\usepackage{graphicx}
\usepackage{tikz}
\usetikzlibrary{arrows.meta,positioning,shapes.geometric,calc,fit,decorations.pathreplacing,patterns,shadows}
\tikzset{
...
% Note: Cyrillic comments in listings under pdfLaTeX may render incorrectly. Use ASCII.
% Convenience macros
...
% Glossary and acronyms (requires makeglossaries/makeindex)
...
\newglossaryentry{syscall}{
  ...
  sort={sistemniy vizov},
  description={механизм запроса приложением привилегированной операции у ядра операционной системы, обычно через инструкцию \texttt{syscall}}
}
