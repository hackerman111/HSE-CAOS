...
\hypersetup{
  ...
  pdftitle={Лекция 2: Файловая система и файловые дескрипторы}
}
...
\newcommand{\LectureTitle}{Файловая система и файловые дескрипторы}
...
\newglossaryentry{syscall}{
  ...
  sort={sistemnyi vyzov},
  description={механизм, с помощью которого пользовательская программа запрашивает у ядра операционной системы выполнение привилегированной операции, например, для работы с устройствами или файлами.}
}
\newglossaryentry{fs}{
...
\newglossaryentry{fd}{
  ...
  sort={failovyi deskriptor},
  description={неотрицательное целое число, служащее идентификатором (<<ручкой>>) для доступа к открытому файлу, каналу или другому ресурсу ввода-вывода в рамках конкретного процесса.}
}
...
% ===================== QC-отчёт ========================================================
% QC:
% - Структура лекции полностью восстановлена по слайдам и транскрипту.
% - Основные темы: проблемы прямого доступа к диску, файловая система как абстракция, права доступа в Linux, файловые дескрипторы и ключевые системные вызовы (open, lseek, dup2, pipe), практические примеры перенаправления и буферизации, работа с директориями.
% - Все определения, примеры кода и концепции взяты строго из предоставленных материалов.
% - Добавлены TikZ-схемы для наглядной иллюстрации устройства HDD и работы dup2, что соответствует стилю "методички" и обосновано содержанием лекции.
% - Глоссарий заполнен ключевыми терминами (файловый дескриптор, системный вызов, umask, pipe и т.д.).
% - Кодовые листинги (open, redirect.cpp, myls) адаптированы из слайдов и объяснений в транскрипте.
% - Добавлены блоки tcolorbox для определений, примечаний и итогов, что улучшает структуру и читаемость.
% - Проверена корректность LaTeX-синтаксиса, уникальность меток и использование пакетов из преамбулы.
% - Рекомендации: Следующая лекция, вероятно, будет посвящена процессам (создание, управление), так как тема pipe была введена как инструмент межпроцессного взаимодействия. Стоит ожидать системные вызовы fork() и exec().
% ======================================================================================
